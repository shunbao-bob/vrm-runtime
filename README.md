...
## Pose disorder after switching models: root cause and solution (located 08-29)

### Symptom
- **First loading** any model → the pose is correct (gltfio's bind pose, the author's baked
  standing pose).
- **After switching models once** (the model picker in the main demo or the
  `dev.vrm.runtime.demo.SWITCH` broadcast) → **the new model's static pose is twisted/
  scrambled** (unnatural hands/legs, especially thighs and arms); but the face orientation
  is roughly fine, and **playing that model's own VRMA animation is normal again**.

### Root cause (not "writes don't work", but "identity NLR written back to the model's rest local")
The main demo's model-switch entry used to run `avatar.execute(AvatarCommand.Reset)` after
`loadModel(newModel)`:

- `Reset` → `AvatarEngineController.reset()` → `humanoid.resetNormalizedPose()` (sets all 55
  **normalized-rig bone quaternions to identity**) → `humanoid.update()`.
- `update()` writes back per-bone local rotations using the VRM 1.0 normalized retargeting
  formula: `PoseForB = L · W⁻¹ · NLR · W`. When **NLR = identity** this degenerates to
  `L · W⁻¹ · W = L`, i.e. it writes each bone's local rotation back to the **rest local
  rotation in the GLB**.
- The problem: for models like **VRoid / xlunar whose rest is an A-pose, the rest local
  carries large rotations** (measured: `J_Bip_L_UpperLeg` rest local ≈ X 170°,
  quat≈(0.996,-0.03,-0.026,0.08)); while gltfio's skin/joint parent chain is its own
  recomputed bind tree, **inconsistent with the GLB rest parent chain**. Hanging the "GLB
  rest local" onto the "gltfio bind parent chain" shifts the world orientation of the
  legs/arms, which looks "scrambled" on screen.

Log evidence (`AvatarEngine`):
- After the switch, `DIAG initial-rot-BEFORE-reset: leftUpperLeg=(1.00,-0.03,-0.03,0.08)`
  — **exactly the GLB rest local** (identical to the rest values read by `glb_inspect` /
  the unit test).
- But the **world coordinates** of hips/feet in `DIAG world:` have always been **normal**
  (feet at y=0.14, plausible standing) — because DIAG reads the headless `liveStore` (GLB
  math), which cannot see gltfio's real rendered values.
- `leftUpperLeg` is extremely sensitive to the parent (hips) world rotation (L is a ~X-170°
  rotation), so the parent-chain deviation is amplified into a large visual offset; arms have
  a near-identity rest L and are insensitive to the parent rotation, so the hands still look
  fine. That explains "why the legs skew the most".
- While a VRMA is playing, each frame overwrites with a **real non-identity NLR** (the actual
  retargeted pose); even if a parent chain has error it only stacks as a relative rotation,
  visually "pressed down" → animation looks normal.

**Conclusion**: `avatar.reset()` / `resetNormalizedPose()` is **dangerous** for models like
VRoid that have a "non-T-pose rest + large local rotations": it forces the bones back to the
GLB rest local, and the whole skeleton skews whenever gltfio's parent chain disagrees with
the GLB's. First load doesn't reset (bones keep gltfio's bind pose), so it's correct.

### The approach currently in use (recommended, effective 2026-08-29)
**The main demo's model-switch entry no longer calls `AvatarCommand.Reset`; it keeps
gltfio's bind pose (same as first load).**
- Change the two spots in `app/.../VrmDemoScreen.kt` (the `SWITCH` broadcast branch + the UI
  model Tab click branch), removing `renderer.avatar?.execute(AvatarCommand.Reset)`.
- Effect: after switching, the bones stay at gltfio's bind pose (as on first load) and the
  standing pose is natural; if there was a VRMA playing before the switch, the model switch
  keeps playing the current animation and the pose is also normal.
- Verified: after installing, switching VRoid B/C / Seed-san etc. shows a normal static pose;
  logcat has no `EVENT reset`.

> Note: the manual "Reset pose" button (`onReset`, via `avatar?.reset()`) is kept; it is
> user-triggered explicitly, not part of the model-switch entry, and is unaffected by this
> change.

### Other optional approaches (and why they are not used)
1. **Use `SetPose("relaxed")` instead of Reset after switching** (demoPhone's approach):
   `relaxed` is explicit arm/standing values (spine + left/rightUpperArm rotZ ±78), going
   through the **normalized channel with explicit overrides**, never "writing back to each
   model's rest local". It never trips the "write back to GLB rest local" pit, which is why
   demoPhone never had this problem. The main demo chose the "keep bind pose" approach above
   because it wants to keep each model author's own standing pose, so this was not used.
   - Pros: unified standing pose, consistent across models; Cons: loses each model author's
     original standing-pose difference, and `relaxed` only controls the upper body/torso, not
     the legs (legs still rely on bind).
2. **Change `resetNormalizedPose` semantics to a true "normalized T-pose"**: make the
   identity-NLR write-back land on the normalized T-pose instead of the GLB rest local. This
   requires reworking the rest compensation in `VRMHumanoidRig.update()` (e.g. not letting
   `L·W⁻¹·N·W` fall back to L when N=I, but making the normalized rig's rest a T-pose). This
   is a deeper engine-layer change and conflicts with three-vrm's "consistent pose mapped
   back to rest" semantics; high risk, low reward (it doesn't solve cross-model
   inconsistency), so it was not adopted.
3. **Root-chain alignment fix**: make gltfio's and the GLB's **parent/root chains
   consistent** (fix how the `Root→Global→Position→Hips` chain is applied), so writing back
   rest local also renders correctly. This is the most "fundamental" fix, but it requires
   fixing the skeleton-root merge at the gltfio layer — a large, engine-layer change that has
   not been done yet (the current approach already satisfies the requirements).
